...
func MavenInfer(ctx context.Context, t rebuild.Target, mux rebuild.RegistryMux, repoConfig *rebuild.RepoConfig) (rebuild.Strategy, error) {
	...
	if !found && sourceJarGuess != nil {
		if dir, found = validateCommit(sourceJarGuess, t, "source jar"); found {
			ref = sourceJarGuess.Hash.String()
		}
	}
	if !found {
		if pomXMLGuess != "" || tagGuess != "" || sourceJarGuess != nil {
			return nil, errors.Errorf("all heuristics failed to find a valid git ref")
		}
		return nil, errors.Errorf("no git ref guess available")
	}
