...
var StableGitProperties = ZipArchiveStabilizer{
	...
	Func: func(zr *MutableZipReader) {
		...
		// We find all the git properties files in the jar file by looking at the pom.xml files.
		// We then remove the content of these files in order to stabilize the jar file.
		var gitPropertiesFiles []string
		for _, zf := range zr.File {
			if strings.HasSuffix(zf.Name, "pom.xml") {
				reader, err := zf.Open()
				if err != nil {
					continue
				}
		...
		if len(gitPropertiesFiles) == 0 {
			// Default git file names generated by git-commit-id-maven-plugin
			addDefaultPaths(&gitPropertiesFiles)
		}

		for _, zf := range zr.File {
			if slices.Contains(gitPropertiesFiles, zf.Name) {
